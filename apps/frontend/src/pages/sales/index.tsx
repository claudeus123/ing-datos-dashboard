...
import { getSaleQuery } from 'src/queries/salesQuery'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const datos = await getSaleQuery(query);
        if (datos) {
          ...
          for (const object of datos) {
            lista_nombres.push(object.categoria);
            lista_cantidad.push(object.total_ventas);
          }
      ...
      try {
        ...
        if (datos) {
          ...
          for (const object of datos) {
            ...
            lista_cantidad.push(object.ingresos_totales);
          }
  ...
  return (
    <ApexChartWrapper>
      <Grid container spacing={6}>
        ...
        <Grid item xs={12} md={6} lg={16}>
          <LineChart title="Ingresos por mes en el año 2022" seriesData={monthlyIncome.amount} categoriesData={monthlyIncome.labels} /> {/* ACA */}
        </Grid>
      </Grid>
    </ApexChartWrapper>
  )
